...
<meta name="author" content="{{blogInfo()->blog_author}}" />
...
<meta name='twitter:title' content="{{blogInfo()->blog_name}}" property="og:title" itemprop="name" />
<meta name='twitter:description' content="{{blogInfo()->blog_description}}" property="og:description" itemprop="description" />
@endsection
        ...
        <div class="row">
            <div class="col-12 mb-4">
                ...
                <article class="card article-card">
                    <a href="{{ route('read_post', single_latest_post()->slug) }}">
                        <div class="card-image">
                            ...
                            <img loading="lazy" decoding="async" src="/storage/images/post_images/thumbnails/resized_{{single_latest_post()->featured_image}}" alt="{{single_latest_post()->post_title}}" class="w-100">
                        </div>
                    </a>
                    <div class="card-body px-0 pb-1">
                        ...
                        <h2 class="h1"><a class="post-title" href="{{ route('read_post', single_latest_post()->slug) }}">
                                {{single_latest_post()->post_title}}
                            </a></h2>
                        <p class="card-text">{!!Str::ucfirst(words(single_latest_post()->post_content, 35))!!}</p>
                        <div class="content"> <a class="read-more-btn" href="{{ route('read_post', single_latest_post()->slug) }}">Read Full Article</a>
                        </div>
                    </div>
                </article>
                @endif
            </div>
            @foreach (lates_home_6post() as $item)
                        ...
                        <div class="card-image">
                            <div class="post-info">
                                ...
                                <span class="text-uppercase">
                                    {{readDuration($item->post_title, $item->post_content) }} @choice('min|mins',
                                    readDuration($item->post_title, $item->post_content)) read
                                </span>
                            </div>
                            <img loading="lazy" decoding="async" src="/storage/images/post_images/thumbnails/resized_{{$item->featured_image}}" alt="{{$item->post_title}}" class="w-100">
                        </div>
                    </a>
                    <div class="card-body px-0 pb-0">
                        <ul class="post-meta mb-2">
                            <li>
                                <a href="{{ route('category_post',$item->subcategory->slug) }}">{{$item->subcategory->subcategory_name}}</a>
                            </li>
                        </ul>
                        <h2><a class="post-title" href="{{ route('read_post',$item->slug) }}">
                                {{$item->post_title}}</a></h2>
                        <p class="card-text">
                            {!!Str::ucfirst(words($item->post_content, 20))!!}
                        </p>
                        <div class="content"> <a class="read-more-btn" href="{{ route('read_post',$item->slug) }}">Read
                                Full Article</a>
                        </div>
                    </div>
                </article>
